...
```bash
php artisan laravelcrm:xero invoices
```

Invoice syncing is currently a placeholder — the infrastructure is in place but the sync logic is not yet implemented.

### Quotes Sync

```bash
php artisan laravelcrm:xero quotes
```

Quote syncing is planned but not yet implemented.

## Scheduling Syncs

To keep data in sync automatically, add the commands to your application's scheduler in `app/Console/Kernel.php`:

```php
$schedule->command('laravelcrm:xero contacts')->hourly();
$schedule->command('laravelcrm:xero products')->hourly();
```
